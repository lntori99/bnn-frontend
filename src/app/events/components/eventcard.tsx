...
export default function EventCard({
  ...
  return (
    ...
    >
      ...
      <div className="p-6">
        <p
          className={`text-xs font-bold uppercase tracking-widest ${past ? "text-clay" : "text-[#d6ac63]"}`}
        >
        ...
        <p className="mt-2 text-xl font-medium tracking-tight">
          <Link
            ...
            className="hover:underline decoration-gold"
          >
            {event.title}
          </Link>
        </p>
        ...
        <p className={`mt-3 text-sm ${past ? "opacity-75" : "text-ivory/80"}`}>
          {event.description}
        </p>
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          <Link
            href={`/events/${event.slug}`}
            className={`inline-flex items-center gap-2 rounded-full border-2 border-current px-6 py-[0.8rem] text-[0.95rem] font-bold transition duration-150 ease-out hover:-translate-y-px focus-visible:outline-[3px] focus-visible:outline-gold focus-visible:outline-offset-2 ${past ? "" : "text-ivory"}`}
          >
            Details
          </Link>
        </div>
